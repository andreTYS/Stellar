...
.float-progress {
  ...
  visibility: hidden !important;   /* No visible en ningún caso */
...
#levelTitle {
  ...
  #readingProgress {
    ...
    top: 0; left: 0;
    height: 3px;
    background: linear-gradient(90deg, #00ffff, #a855f7, #ffd700);
    z-index: 9999;
    width: 0%;
  ...
  .dialogue strong, .dialogue b {
    color: var(--primary);
    font-style: normal;
  }

  /* ===== COLLAPSIBLE EDUCATION BOX ===== */
  .edu-toggle {
    width: 100%;
    background: none;
    border: none;
  ...
  .edu-toggle:hover { opacity: 0.85; }
  .edu-chevron {
    stroke: var(--accent);
    flex-shrink: 0;
    transition: transform 0.35s cubic-bezier(0.4,0,0.2,1);
  }
  .edu-toggle[aria-expanded="true"] .edu-chevron {
...
<body>
  ...
  <header id="topbar" aria-hidden="false">
    <div class="topbar-inner">
      ...
      <div class="mini-nav" role="toolbar" aria-label="controles">
        ...
        <button class="nav-pill" id="music-toggle">🎵</button>
        <button class="nav-pill" id="mute-narration">🔈</button>
        <button class="nav-pill" id="fullscreen">⛶</button>
      </div>
    </div>
  </header>
  ...
  <div id="startScreen" class="start-screen" aria-hidden="false">
    <div class="start-card">
      ...
      <div class="start-actions">
        <button id="startBtn" class="btn-primary">INICIAR MISIÓN</button>
  ...
  /* ===== PROGRESS BAR ===== */
  const TOTAL_CHAPTERS = Object.keys(story).length; // 4
  function updateProgress(n) {
    const pct = Math.min(100, Math.round((n - 1) / TOTAL_CHAPTERS * 100));
    document.getElementById('readingProgress').style.width = pct + '%';
    document.getElementById('sceneCounter').textContent = `CAPÍTULO ${n} / ${TOTAL_CHAPTERS}`;
  }
  ...
  async function renderChapter(n) {
      ...
      <article class="scene visible" id="scene-${n}">
        ...
        <div class="content">
          ...
          <div class="education-box">
            <button class="edu-toggle" aria-expanded="false" id="eduToggle">
              <svg class="edu-chevron" viewBox="0 0 24 24" width="18" height="18" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><polyline points="6 9 12 15 18 9"/></svg>
              <span>${ch.education.title}</span>
              <span class="edu-hint">expandir</span>
            </button>
            <div class="edu-body" id="eduBody">
              <ul style="margin-top:14px">${ch.education.points.map(p=>`<li>${p}</li>`).join('')}</ul>
            </div>
          </div>
        ...
        <div class="media" style="align-items:center;justify-content:center">
          <img id="sceneImg" class="media-img" src="${ch.leftChar}" alt="Escena">
        </div>
      </article>
    `;
    ...
    // Track chapter progress in DB (silently; no-op if not logged in)
    ...
    // Wire collapsible education box
    ...
    if (eduToggle && eduBody) {
      eduToggle.addEventListener('click', () => {
        const isOpen = eduToggle.getAttribute('aria-expanded') === 'true';
        eduToggle.setAttribute('aria-expanded', isOpen ? 'false' : 'true');
        eduBody.classList.toggle('open', !isOpen);
        eduToggle.querySelector('.edu-hint').textContent = isOpen ? 'expandir' : 'contraer';
      });
    }
    ...
    async function revealBlock() {
      ...
      // Crossfade scene image on each block reveal
      const sceneImg = document.getElementById('sceneImg');
      if (sceneImg) {
        const nextSrc = imgToggle ? ch.rightChar : ch.leftChar;
        imgToggle = !imgToggle;
        sceneImg.style.opacity = '0';
        setTimeout(() => { sceneImg.src = nextSrc; sceneImg.style.opacity = '1'; }, 380);
      }
      ...
      cursor = end;
      continueBtn.textContent = cursor >= paragraphs.length ? 'SIGUIENTE CAPÍTULO →' : 'CONTINUAR →';
      scrollMainTop();
    }
  ...
  function renderBranchPoint() {
    d.levelBadge.textContent = 'DECISIÓN';
    d.levelTitle.textContent = 'Punto de Inflexión';
    document.getElementById('readingProgress').style.width = '90%';
    document.getElementById('sceneCounter').textContent = 'DECISIÓN FINAL';
    fadeIn();
    ...
    setTimeout(() => {
      ...
      speak('Has llegado a un punto de decisión. Escoge con sabiduría.', 'narrator');
      fadeOut(250);
      scrollMainTop();
    }, 400);
  }
  ...
  async function selectBranch(b) {
    ...
    d.gameArea.innerHTML = `
      <article class="scene visible">
        <div class="bg" style="background-image:url('img/bg-anim.jpg');"></div>
        <div class="overlay"></div>
        <div class="content">
          <h2 class="page-title">Ramificación ${b}</h2>
  ...
  async function showConclusion() {
    document.getElementById('readingProgress').style.width = '100%';
    document.getElementById('sceneCounter').textContent = 'FIN';
    fadeIn();
    await new Promise(r=>setTimeout(r, 350));
  ...
  d.muteNarration.onclick = () => {
    ...
    d.muteNarration.textContent = config.narrationOn ? '🔈' : '🔕';
  ...
  console.log('🚀 Aldrin NASA Visual Premium Edition v2.0');
  console.log('✨ Características: Diseño premium, animaciones avanzadas, efectos de partículas');
  console.log('🎯 Optimizado para NASA Space Apps Challenge 2025');
  </script>
</body>
</html>
